...
func generateHbaConf(cm *configmaps.ConfigMap) string {
	...
	for _, v := range cm.Spec.HbaConf {
		config = config + fmt.Sprintf("%s\t%s\t%s\t%s\t%s\t%s\n", v.Type, v.Database, v.User, v.Address, v.IpMask, v.Method)
	}
...
func generatePostgresqlConf(cm *configmaps.ConfigMap) string {
	...
	config = config + fmt.Sprintf("hba_file = %s\n", cm.Spec.PGConf.HbaFile)
	config = config + fmt.Sprintf("max_connections = %d\n", cm.Spec.PGConf.MaxConnections)
	config = config + fmt.Sprintf("port = %d\n", cm.Spec.PGConf.Port)
	return config
}
